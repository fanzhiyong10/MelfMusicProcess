...
class MelfPowerMeter: MelfAudioLevelProvider {
    ...
    private var meterTableAvarage = MelfMeterTable()
    private var meterTablePeak = MelfMeterTable()

    var levels: MelfAudioLevels {
        if values.isEmpty { return MelfAudioLevels(level: 0.0, peakLevel: 0.0) }
        return MelfAudioLevels(level: meterTableAvarage.valueForPower(values[0].average),
