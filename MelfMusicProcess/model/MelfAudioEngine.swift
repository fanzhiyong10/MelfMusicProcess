//
//  MelfAudioEngine.swift
//  MelfMusicProcess
//
//  Created by 范志勇 on 2022/9/16.
//

import AVFoundation
import Foundation

class MelfAudioEngine {
    ...
    public private(set) var speechPowerMeter = MelfPowerMeter()
    public private(set) var fxPowerMeter = MelfPowerMeter()
    public private(set) var voiceIOPowerMeter = MelfPowerMeter()
    
    func setVolumeOfFxPlayer(_ value: Float=1.0) {
        fxPlayer.volume = value
    }

    func setVolumeOfSpeechPlayer(_ value: Float=1.0) {
        speechPlayer.volume = value
    }
    ...
    init() throws {
        ...
        guard let tempSpeechBuffer = MelfAudioEngine.getBuffer(fileURL: speechURL) else { throw AudioEngineError.bufferRetrieveError }
        ...
        guard let tempFxBuffer = MelfAudioEngine.getBuffer(fileURL: fxURL) else { throw AudioEngineError.bufferRetrieveError }
    ...
    func togglePlaying() {
        if recordedFilePlayer.isPlaying {
            ...
            if isNewRecordingAvailable {
                guard let recordedBuffer = MelfAudioEngine.getBuffer(fileURL: recordedFileURL) else { return }
