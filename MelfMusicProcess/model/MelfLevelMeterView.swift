...
struct MelfAudioLevels {
    let level: Float
    let peakLevel: Float
}

// The protocol for the object that provides peak and average power levels to adopt.
protocol MelfAudioLevelProvider {
    var levels: MelfAudioLevels { get }
}

class MelfLevelMeterView: UIView {
    ...
    // The class that provides the audio meter data needs to adopt MelfAudioLevelProvider.
    var levelProvider: MelfAudioLevelProvider?
